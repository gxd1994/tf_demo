...
def convs_comm(x, depth, num_output, wd, is_training, transfer_mode= False):
    stages= []
    K=32
    if depth == 121:
      stages= [6, 12, 24, 16]
    ...
    return getModel_comm(x, num_output, K, stages, wd, is_training, transfer_mode= transfer_mode)
...
def getModel_comm(x, num_output, K, stages, wd, is_training, transfer_mode= False):
    with tf.variable_scope('conv1'):
        x = common.spatialConvolution(x, 7, 2, 2*K, wd= wd)
        x = common.batchNormalization(x, is_training= is_training)
        x = tf.nn.relu(x)
